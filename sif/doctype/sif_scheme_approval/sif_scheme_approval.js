...
frappe.ui.form.on("SIF Scheme Approval", {
	setup(frm) {
		frappe.call({
			method: "dhanada.sif.doctype.sif_scheme_approval.sif_scheme_approval.get_editable_fields",
			callback: function(r) {
				if (r.message) {
	...
	apply_dynamic_options(frm) {
		...
		let base_df = frappe.meta.get_docfield("SIF Scheme Approval Item", "field_name");
		if (base_df) base_df.options = options;
		
		let meta_df = frappe.meta.get_docfield("SIF Scheme Approval Item", "field_name", frm.doc.name);
